...
//         // 1) clusterResultQueue → setCoverReadyQueue 이동
...
//             // 크기가 변했으면 재삽입 후 다시 확인
//             if (valid.size() != res.memberIds.size()) {
...
void clusterThreadFuncSetcover(unsigned int* assignedCluster) {
    std::vector<unsigned int> valid;  // 루프 밖에서 한 번만 할당
    valid.reserve(1024);  // 적당한 초기 크기
    ...
    while (true) {
        ...
        while (!setCoverReadyQueue.empty() &&
            ...
            if (valid.size() != originalSize) {
                res.memberIds.swap(valid);  // move 대신 swap (valid 재사용 위해)
                setCoverReadyQueue.push(std::move(res));
                continue;
            }
...
int doAlign2clust(Parameters &par, DBWriter &resultWriter, DBReader<unsigned int> &alnDbr, const size_t dbSize) {
    ...
    // mode에 따라 적절한 클러스터 함수 선택
    if (mode == Parameters::SET_COVER_STATIC) {
...
#pragma omp for schedule(dynamic, 1000)
    ...
    size_t endRange = (mode == Parameters::SET_COVER_STATIC) ? dbSize : alnDbr.getSize();
...
int align2clust(int argc, const char **argv, const Command &command) {
    ...
    DBWriter resultWriter(par.db4.c_str(), par.db4Index.c_str(), 1, par.compressed, dbtype);
    resultWriter.open();

    int status = doAlign2clust(par, resultWriter, alnDbr, alnDbr.getSize());
